...
type Props<T extends number | [number, number]> = React.ComponentProps<
  typeof BaseSlider<T>
> & {
  label?: string
  unit?: string
...
const InputField = ({ value, onChange, unit, disabled }: Props<number>) => {
  return (
    <Field>
      <Input
        type="number"
...
const Unit = styled.div`
  ...
  line-height: 144%;
`
const Slider = styled(BaseSlider<[number, number]>)`
  width: calc(100% - 32px);
  position: absolute;
  left: 16px;
  bottom: -1px;
...
export const FieldInputRange = React.forwardRef<
  React.ComponentRef<typeof InputRange>,
  Pick<Props<[number, number]>, "label"> &
    FieldRenderProps<
